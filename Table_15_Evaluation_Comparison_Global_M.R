...
covariates_reduced <- covariates_reduced[154:nrow(covariates_reduced),1:60]
...
# ARFIMA
arfima_3 <- arfima(train_3$`GCPU(PPP-adjusted GDP)`)
...
# ARIMA
arima_3 <- auto.arima(train_3$`GCPU(PPP-adjusted GDP)`)
...
# ARNN
arnn_3 <- nnetar(train_3$`GCPU(PPP-adjusted GDP)`, xreg = train_reg_reduced_3, MaxNWts = 5500)
...
bsts_3 <- bsts(cpu_index ~ ., 
               ...
               ping = 100,
               seed = 100,
               data = as.data.frame(cbind(train_3$`GCPU(PPP-adjusted GDP)`, train_reg_reduced_3)) %>% rename('cpu_index' = V1))
...
setwd('Climate_Policy_Uncertainty_Forecasting/Dataset/Dataset_Deep_Learning_Models_Forecasts/Models - Macroeconomic/Global')
...
model_evaluate_3 <- rbind(model_evaluate_3, evaluate(test_3, cpu_3_NHiTS, model = 'NHiTS'))
...
# ARFIMA
arfima_6 <- arfima(train_6$`GCPU(PPP-adjusted GDP)`)
...
# ARIMA
arima_6 <- auto.arima(train_6$`GCPU(PPP-adjusted GDP)`)
...
# ARNN
arnn_6 <- nnetar(train_6$`GCPU(PPP-adjusted GDP)`, xreg = train_reg_reduced_6, MaxNWts = 5700)
...
# ARFIMA
arfima_12 <- arfima(train_12$`GCPU(PPP-adjusted GDP)`)
...
# ARIMA
arima_12 <- auto.arima(train_12$`GCPU(PPP-adjusted GDP)`)
...
# ARNN
arnn_12 <- nnetar(train_12$`GCPU(PPP-adjusted GDP)`, xreg = train_reg_reduced_12, MaxNWts = 5600)
...
bsts_12 <- bsts(cpu_index ~ .,
                ...
                ping = 100,
                seed = 100,
                data = as.data.frame(cbind(train_12$`GCPU(PPP-adjusted GDP)`, train_reg_reduced_12)) %>% rename('cpu_index' = V1))
...
# ARFIMA
arfima_24 <- arfima(train_24$`GCPU(PPP-adjusted GDP)`)
...
# ARIMA
arima_24 <- auto.arima(train_24$`GCPU(PPP-adjusted GDP)`)
...
# ARNN
arnn_24 <- nnetar(train_24$`GCPU(PPP-adjusted GDP)`, xreg = train_reg_reduced_24, MaxNWts = 5500)
...
bsts_24 <- bsts(cpu_index ~ .,
                ...
                ping = 100,
                seed = 100,
                data = as.data.frame(cbind(train_24$`GCPU(PPP-adjusted GDP)`, train_reg_reduced_24)) %>% rename('cpu_index' = V1))
